  ...
  return (
    <div className="authShell">
      <div className="authCard">
        ...
        <p>
          Your assistant-coach account is awaiting approval
          from Coach Christian.
        </p>
        <button
          className="primary"
  ...
  <a href="/coach/registrations" className="btn">
    Manage Registrations
       ...
       <a href="/coach/progress" className="btn">Player Progress</a> <nav className="tabs"><button className={tab==='dashboard'?'active':''} onClick={()=>setTab('dashboard')}>Dashboard</button><button className={tab==='create'?'active':''} onClick={()=>setTab('create')}>Create Assignment</button><button className={tab==='players'?'active':''} onClick={()=>setTab('players')}>Players</button></nav>
        ...
        {tab === 'create' && <CreateAssignment userId={profile.id} teams={teams} isAdmin={isAdmin} onCreated={() => { loadData(profile.id); setTab('dashboard') }} setMessage={setMessage} />}
...
function CoachDashboard({teams,assignments,submissions,managedSubmissions,playerCount,submittedCount,onOpen,selected,questions,tasks}:{teams:Team[];assignments:Assignment[];submissions:Submission[];managedSubmissions:{ assignment_id:string; status:string }[];playerCount:number;submittedCount:number;onOpen:(id:string)=>void;selected:string|null;questions:Question[];tasks:Task[]}){
 return <><div className="metrics"><Metric label="Players" value={playerCount}/><Metric label="Teams" value={teams.length}/><Metric label="Assignments" value={assignments.length}/><Metric label="Submitted" value={submittedCount}/></div><section className="card section"><div className="sectionHead"><div><h2>Assignments</h2><p className="muted">Track what your players are working on.</p></div></div>{assignments.length===0?<Empty text="No assignments yet. Create your first Watch → Think → Train assignment."/>:assignments.map(a=>{const team=teams.find(t=>t.id===a.team_id);const subs = submissions.filter(s => s.assignment_id === a.id);const managedDone = managedSubmissions.filter(

  s => s.assignment_id === a.id && s.status === 'completed'

).length;const done=subs.filter(s=>['submitted','reviewed'].includes(s.status)).length + managedDone;return <div className="assignmentRow" key={a.id}><div className="grow"><div className="eyebrow">{team?.name||'Team'} · {a.status}</div><h3>{a.title}</h3><div className="muted">Due {fmtDue(a.due_at)} · {done} submitted</div><div className="progress"><i style={{width:`${subs.length + managedSubmissions.filter(s => s.assignment_id === a.id).length?Math.round(done/(subs.length + managedSubmissions.filter(s => s.assignment_id === a.id).length)*100):0}%`}}/></div></div><a className="secondary" href="/coach/progress">View Progress</a></div>})}</section>{selected&&<section className="card section"><h2>Assignment content</h2>{questions.map((q,i)=><div className="contentLine" key={q.id}><b>Question {i+1}</b><span>{q.prompt}</span></div>)}{tasks.map((t,i)=><div className="contentLine" key={t.id}><b>Training {i+1}</b><span>{t.description}</span></div>)}</section>}</>
}
